import { CheckCircle, Clock, Download, Hash } from "lucide-react";
...
export default async function HonorPage() {
  ...
  return <main className="admin-main"><div className="page-heading"><div><h1>Honorarer</h1><p className="text-muted">{viewAll ? "Samlet fakturagrundlag" : "Dine honorarer"}</p></div>{viewAll && <a className="btn btn-secondary" href="/api/honorar/export"><Download size={17} /> Eksportér CSV</a>}</div>
    <div className="honor-summary"><article className="card"><Clock size={22} /><span>Afventer godkendelse</span><strong>{pending.toLocaleString("da-DK")} kr.</strong></article><article className="card"><CheckCircle size={22} /><span>Godkendt/eksporteret</span><strong>{approved.toLocaleString("da-DK")} kr.</strong></article><article className="card"><Hash size={22} /><span>Antal posteringer</span><strong>{entries.length}</strong></article></div>
