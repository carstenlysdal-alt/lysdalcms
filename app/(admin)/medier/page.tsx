...
export default async function MediaPage({ searchParams }: PageProps<"/medier">) {
  ...
  return <main className="admin-main">
    <div className="page-heading"><div><h1>Mediebibliotek</h1><p className="text-muted">{media.length} medier i den aktuelle visning</p></div>{can(session.user, PERMISSIONS.MEDIA_MANAGE) && <Link className="btn btn-primary" href="/medier/ny"><Plus size={17} /> Tilføj medie</Link>}</div>
